...
const Videos = () => {
  ...
  const videoTitles=[
    'The city merges with the sky beyond the horizon',
    'Peaceful life of a frog - no assignments, no exams',
    'Time flies, yet it heals',
    'Paragliding - a rejuvenating sport',
    'A morning troll amidst the woods is refreshing',
    'A big wave is like a beautiful woman, exciting to look at and thrilling to ride',
    'Where the land and the sea meets'
  ]
  
  const [videoSrc, setVideoSrc] = useState(videosArr[0]);
  const [videoNo, setVideoNo]=useState(1);
  const [videoThumbnail, setVideoThumbnail]=useState(videoTitles[0]);
  ...
  return (
    <Stack direction={['column', 'row']} h={'100vh'}>
      <VStack w={'full'}>
        ...
        <VStack alignItems={'flex-start'} p={'8'} w={'full'} overflowY={'auto'} borderBottom={'1px solid black'}>
          <Heading> {videoThumbnail} </Heading>
          <Text>
      ...
      <VStack
        ...
        overflowY={'auto'}
      >
        <Heading>View your lectures here</Heading>
        {videosArr.map((item, index) => (
          <Button
            ...
            onClick={() => {
                setVideoSrc(item)
                setVideoNo(index+1)
                setVideoThumbnail(videoTitles[index])
            }}
          >
            Lecture {index + 1}
          </Button>
